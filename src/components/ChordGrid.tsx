...
export const GRID_BARS = 8;
const BEATS_PER_BAR = 4;
const BARS_PER_ROW = 4;
const BEATS_PER_ROW = BARS_PER_ROW * BEATS_PER_BAR; // 16
export const TOTAL_BEATS = GRID_BARS * BEATS_PER_BAR; // 32
const ROWS = GRID_BARS / BARS_PER_ROW; // 2

const BEAT_WIDTH = 28; // px — also used for resize/move/loop-drag pixel-to-beat math
...
function clientPosToGlobalBeat(wrapperRect: DOMRect, clientX: number, clientY: number): number {
  ...
  const beatInRow = clamp(Math.floor((clientX - wrapperRect.left) / BEAT_WIDTH), 0, BEATS_PER_ROW - 1);
...
export function ChordGrid({
  ...
  const handleResizeStart = (placement: ChordPlacement) => (e: ReactMouseEvent) => {
    ...
    const handleMouseMove = (moveEvent: MouseEvent) => {
      const deltaBeats = Math.round((moveEvent.clientX - startX) / BEAT_WIDTH);
  ...
  return (
    <div className="chord-grid-wrapper">
      ...
      <div ref={wrapperRef} className="chord-grid" onDragOver={handleDragOver} onDrop={handleDrop}>
        {Array.from({ length: ROWS }, (_, row) => {
          ...
          const rowWidth = BEATS_PER_ROW * BEAT_WIDTH;

          return (
            <div key={row} className="chord-grid-row-group">
              <div className="loop-ruler" style={{ width: rowWidth }}>
                {loopStartRow === row && (
                  <div
                    className="loop-handle loop-handle-start"
                    style={{ left: (loopStart - rowStart) * BEAT_WIDTH }}
                ...
                {loopEndHomeRow === row && (
                  <div
                    ...
                    style={{ left: (loopEnd - rowStart) * BEAT_WIDTH }}
              ...
              <div
                ...
                style={{ gridTemplateColumns: `repeat(${BEATS_PER_ROW}, ${BEAT_WIDTH}px)` }}
              >
                ...
                {playheadRow === row && playheadBeat !== null && (
                  <div
                    className="playhead"
                    style={{ left: (playheadBeat - rowStart) * BEAT_WIDTH }}
                  />
                )}
                {rowPlacements.map((placement) => {
                  ...
                  return (
                    ...
                    >
                      <div className="chord-block-body" onMouseDown={handleMoveStart(placement)}>
